...
const MAX_EXPORT_ONLINE_ORDERS = 25000;
...
const buildOnlineOrderWorkbook = async ({ orders = [], meta = {} } = {}) => {
    ...
    // ── Orders ────────────────────────────────────────────
    const orderDefs = [
        { h: "Order ID", k: "text", w: 24, g: C.gProduct, groupStart: true },
        ...
        { h: "Email", k: "text", w: 22, g: C.gCatalog },
        { h: "Phone", k: "text", w: 14, g: C.gCatalog },
        { h: "Address", k: "text", w: 32, g: C.gCatalog },
        { h: "City", k: "text", w: 14, g: C.gCatalog },
        { h: "Country", k: "text", w: 12, g: C.gCatalog },
        { h: "Branch", k: "text", w: 16, g: C.gStock, groupStart: true },
        { h: "Payment", k: "center", w: 12, g: C.gMoney, groupStart: true },
        { h: "Coupon", k: "text", w: 12, g: C.gMoney },
        { h: "Items", k: "qty", w: 8, g: C.gStock, groupStart: true },
        { h: "Subtotal", k: "money", w: 12, g: C.gMoney, groupStart: true },
        { h: "Discount", k: "money", w: 11, g: C.gMoney },
        { h: "Total", k: "money", w: 12, g: C.gMoney },
        { h: "Tracking URL", k: "text", w: 28, g: C.gMeta, groupStart: true },
    ];

    const orderSheet = buildSheetChrome(
        workbook,
        "Orders",
        "FF0D9488",
        orderDefs,
        "  Lowkia ERP   ·   Online Orders",
        `  Storefront checkout orders    ·    ${fmtDateTime(exportedAt)}`
    );

    let or = orderSheet.headerRow;
    let oz = 0;
    let sumSub = 0;
    let sumDisc = 0;
    let sumTotal = 0;
    ...
    for (const o of orders) {
        ...
        const subtotal = num(o.orderTotal?.subtotal ?? o.totalPrice);
        const discount = num(o.orderTotal?.discount);
        const total = num(o.orderTotal?.total ?? o.totalPrice);
        sumSub += subtotal;
        sumDisc += discount;
        sumTotal += total;
        sumItems += itemCount;
        const st = str(o.orderStatus) || "unknown";
        ...
        appendDataRow(
            ...
            [
                ...
                str(o.orderStatus),
                userLabel(o.userID),
                typeof o.userID === "object" ? str(o.userID.email) : "",
                str(addr.phone),
                addressLine(addr),
                str(addr.city),
                str(addr.country),
    ...
    or += 1;
    orderSheet.sheet.getRow(or).height = 8;
    fillRow(orderSheet.sheet, or, 1, orderSheet.colCount, C.canvas);
    or += 1;
    appendTotalsRow(orderSheet.sheet, or, orderDefs, [
        "TOTALS",
    ...
    freezeFilter(
        orderSheet.sheet,
        orderSheet.headerRow,
        orderSheet.colCount,
        Math.max(orderSheet.headerRow, or - 2)
    );

    // ── Line Items ────────────────────────────────────────
    const itemDefs = [
        { h: "Order ID", k: "text", w: 24, g: C.gProduct, groupStart: true },
        { h: "Order Date", k: "center", w: 12, g: C.gMeta },
        { h: "Status", k: "status", w: 14, g: C.gStatus, groupStart: true },
        { h: "Customer", k: "text", w: 18, g: C.gCatalog, groupStart: true },
        { h: "Product", k: "text", w: 26, g: C.gCatalog, groupStart: true },
        { h: "Variant", k: "text", w: 16, g: C.gCatalog },
        { h: "Qty", k: "qty", w: 8, g: C.gStock, groupStart: true },
        { h: "Unit Price", k: "money", w: 12, g: C.gMoney, groupStart: true },
        { h: "Line Total", k: "money", w: 12, g: C.gMoney },
        { h: "IMEIs", k: "text", w: 36, g: C.gStock, groupStart: true },
    ];
    ...
    const itemSheet = buildSheetChrome(
        ...
        "FF0369A1",
        itemDefs,
        "  Lowkia ERP   ·   Online Order Line Items",
        `  Products and IMEIs    ·    ${fmtDateTime(exportedAt)}`
    );
    ...
    for (const o of orders) {
        ...
        for (const it of items) {
            ...
            lineQty += qty;
            lineValue += line;
            const imeis = Array.isArray(it.imeis)
                ? it.imeis.filter(Boolean).join(", ")
                : str(it.imeis);
            ir += 1;
            appendDataRow(
                itemSheet.sheet,
    ...
    st.value = "  Lowkia ERP   ·   Online Orders Export Summary";
    st.font = {
        name: "Calibri",
        size: 18,
        bold: true,
    ...
    card(5, 4, "UNITS", sumItems);
    card(5, 6, "GROSS TOTAL", sumTotal, true);
    card(5, 8, "DISCOUNTS", sumDisc, true);
...
const buildExportFilename = (query = {}) => {
    ...
    const status = str(query.status || query.orderStatus)
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "_")
        .replace(/^_|_$/g, "");
    if (status && status !== "all") {
        return `online_orders_${status}_${today}.xlsx`;
    }
    return `online_orders_${today}.xlsx`;
};
